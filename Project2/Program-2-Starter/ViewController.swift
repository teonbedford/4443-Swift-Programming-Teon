...
class ViewController: UIViewController , ColorTableViewControllerDelegate{
    ...
    func ColorTableViewChooseColor(controller: ColorTableViewController, chosenColor color: String){
        ...
        SreenLabel.textColor = labelColor
        ...
        hsllabel.text = hslstring
        
        let hsv = myColors.fetchHSV(color)
        var hsvstring = "\(hsv.H) , \(hsv.S) , \(hsv.V)"
        hsvlabel.text = hsvstring
        ...
        for colors in history {
            ...
            label = UILabel(frame: CGRectMake(50, 500, 20, 20))
            label.center = CGPointMake(CGFloat(x), 500)
            label.textAlignment = NSTextAlignment.Center
            label.backgroundColor = colors
        ...
        println("The number of colors are: \(history.count)")
